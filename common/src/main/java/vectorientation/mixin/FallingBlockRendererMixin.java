...
public class FallingBlockRendererMixin {
	...
	public void addRotation(FallingBlockEntity fallingBlockEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, CallbackInfo info) {
		...
		Vec3f vel = new Vec3f((float) velD.getX(), (float) velD.getY(), (float) velD.getZ());
		float y = vel.getY();
		y -= .04D * g;
		y *= .98D;
		float speed = Vectorientation.squetch ? 0.75f + (float) Math.sqrt(vel.getX() * vel.getX() + y * y + vel.getZ() * vel.getZ()) : 1;
		float angle = (float) Math.acos(y);
		Vec3f axis = new Vec3f(-1 * vel.getZ(), 0, vel.getX());
		Quaternion rot = new Quaternion(Quaternion.IDENTITY);
		if(axis.getY() > .01f){
			axis.normalize();
			rot = new Quaternion(axis, -angle, false);
		}
